...
人間の明示操作後に行うもの: Ollama呼出し、PDF抽出、4分析、Embedding、クラスタリング、結果の原子的保存。本番リサーチの一括処理前に、承認済み少数文献で品質を確認する。

### Phase 5 実証記録

実PDFを用いた接続実証の入力、抽出物、モデル応答、Embedding、実行記録は端末固有の検証資産としてGitへ含めない。実証結果は環境ごとの受入記録へ保存し、複数文献へ拡大する前に請求項照合と要約品質をレビューする。

### Phase 6: リサーチ単位・段階パイプライン

単発実証用の一括プロンプトは後方互換のため残し、複数文献の通常処理には `tools/run_research_pipeline.py` を使用する。昨晩の長時間バッチで複雑なチャンク読解JSONと構成要件JSONが出力上限に達したため、この方式は廃止した。通常処理は、共有抽出、規則ベースの章・請求項分割、類似度、権利範囲の広さ、課題要約、技術要約、2種Embedding、リサーチ全体クラスタ、短いクラスタ名、UI結果確定に分離する。LLMの各出力は1～2項目の小さなJSON Schemaで生成制約し、Pythonでも再検証する。

コードと契約の実装後は、環境ごとに承認済み少数文献で新方式の `execute` を検証する。Schema適合、Embedding、クラスタ命名、UI結果確定、失敗・スキップを受入記録へ残す。複数文献の `execute` は長時間実行と人手品質承認を伴うため、少数件の確認後に進める。詳細設計は [RESEARCH_PIPELINE_DESIGN.md](RESEARCH_PIPELINE_DESIGN.md) を正とする。
