...
5. `concept_level`：独立請求項の限定の少なさをLLMが1～5と短い理由で返す。`concept_level.json`。
6. `summaries`：課題要約と技術要約を別々のLLM要求で作り、Pythonが `summaries.json` に統合する。
7. `embeddings`：各文献と自社基準の技術要約・課題要約を別々にベクトル化する。文献側は `embeddings.json`、自社側はrun単位の `company_reference.json`。
